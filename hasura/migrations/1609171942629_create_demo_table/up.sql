...
CREATE FUNCTION public_posts_search(search text)
RETURNS SETOF public_posts AS $$
    SELECT *
    FROM public_posts
    WHERE
      title ilike ('%' || search || '%')
...
CREATE OR REPLACE VIEW "public"."public_posts_count" AS
  SELECT
    public_posts.created_by,
    count(*) AS posts
  FROM
    public_posts
  GROUP BY
